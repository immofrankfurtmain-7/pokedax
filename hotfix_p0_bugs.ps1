# Hotfix P0: Kartendetail + Profil + Leaderboard + pages_misc
$root = "C:\Users\lenovo\pokedax\pokedax\pokedax"
$enc  = New-Object System.Text.UTF8Encoding $true
Write-Host ""
Write-Host "Hotfix P0-Bugs" -ForegroundColor Yellow
Write-Host "==============" -ForegroundColor Yellow
Write-Host "  Kartendetailseite: duplicate const now fix" -ForegroundColor Green
Write-Host "  Profil: async params Next.js 15" -ForegroundColor Green  
Write-Host "  Leaderboard: echte DB-Daten statt Fake" -ForegroundColor Green
Write-Host "  pages_misc.tsx: undefined vars entfernt" -ForegroundColor Green
Write-Host "  Fantasy + Portfolio LB API: shape fix" -ForegroundColor Green
Write-Host ""
...
# Remove pages_misc if it exists in project (it's not a real page)
$miscPaths = @(
  "$root\src\app\pages_misc.tsx",
  "$root\src\pages_misc.tsx",
  "$root\src\components\PagesMisc.ts"
)
foreach ($p in $miscPaths) {
  if (Test-Path $p) { Remove-Item $p -Force; Write-Host "  DEL pages_misc ($p)" -ForegroundColor Yellow }
}

Write-Host ""
Write-Host "==============" -ForegroundColor Yellow
Write-Host "Fertig!" -ForegroundColor Yellow
Write-Host "GitHub Desktop -> Commit 'fix: P0 bugs - leaderboard + card detail + profil params' -> Push" -ForegroundColor Yellow
