# PokeDax v44 - Exakte Vorlage: Karten + Scanner/Chart + Forum + Pricing + Trennlinien
$root = "C:\Users\lenovo\pokedax\pokedax\pokedax"
$enc  = New-Object System.Text.UTF8Encoding $true
Write-Host "PokeDax v44 - Vollstaendige Startseite" -ForegroundColor Cyan
Write-Host "=======================================" -ForegroundColor Cyan
Write-Host ""

$dirs = @(
  "$root\src\app","$root\src\app\preischeck","$root\src\app\dashboard",
  "$root\src\app\top-movers","$root\src\app\forum",
  ...
  "$root\src\app\api\stats","$root\src\components\layout",
  "$root\src\components\ui","$root\src\components\premium","$root\public"
)
foreach ($d in $dirs) {
...
$globals_css = @'
@import url("https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,300;0,14..32,400;0,14..32,500;0,14..32,600;1,14..32,300&family=DM+Mono:wght@400;500&display=swap");
@tailwind base;
@tailwind components;
@tailwind utilities;
...
:root {
  ...
  /* ── Borders ── */
  --br-1: rgba(255,255,255,0.050);
  --br-2: rgba(255,255,255,0.085);
  --br-3: rgba(255,255,255,0.130);
  ...
  /* ── Gold (single accent, 5 intensities) ── */
  --gold:     #E9A84B;
  --gold-30:  rgba(233,168,75,0.30);
  --gold-18:  rgba(233,168,75,0.18);
  --gold-10:  rgba(233,168,75,0.10);
  --gold-06:  rgba(233,168,75,0.06);
...
::-webkit-scrollbar-thumb:hover { background: var(--br-3); }

'@
[System.IO.File]::WriteAllText("$root\src\app\globals.css", $globals_css, $enc)
...
'@
[System.IO.File]::WriteAllText("$root\src\app\layout.tsx", $layout_tsx, $enc)
...
'@
[System.IO.File]::WriteAllText("$root\src\components\layout\Navbar.tsx", $navbar_tsx, $enc)
...
// Section divider — dünne Linie wie in Vorlage
const Divider = () => (
...
function Feat({ text, off }: { text: string; off?: boolean }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:11.5, color: off?"#474664":"#8C8BAA", textDecoration: off?"line-through":"none" }}>
      <div style={{ width:14, height:14, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, background: off?"rgba(255,255,255,0.03)":"rgba(77,191,133,0.12)" }}>
        {!off && <svg width="7" height="7" viewBox="0 0 8 8"><polyline points="1,4 3,6 7,1.5" stroke="#4DBF85" strokeWidth="1.3" fill="none"/></svg>}
      </div>
      {text}
    </div>
  );
...
async function getData() {
  try {
    ...
    return {
      stats:{ cards:cardsRes.count??22271, users:usersRes.count??3841, scans:1247, forum:forumCountRes.count??18330 },
      cards:trendRes.data??[],
      posts:forumRes.data??[],
      portCards:portRes.data??[],
    };
  } catch {
    return { stats:{cards:22271,users:3841,scans:1247,forum:18330}, cards:[], posts:[], portCards:[] };
  }
}
...
export default async function HomePage() {
  ...
  const G = "#E9A84B";
  const S = { bg:"#111122", br:"rgba(255,255,255,0.085)" };

  return (
    <div style={{ color:"#EDEAF6", maxWidth:1100, margin:"0 auto" }}>

      {/* ══ HERO ════════════════════════════════════════════════ */}
      <section style={{ padding:"72px 24px 60px", textAlign:"center", background:"radial-gradient(ellipse 65% 45% at 50% 0%,rgba(233,168,75,0.055),transparent 70%)" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"4px 12px", borderRadius:20, marginBottom:28, border:"1px solid rgba(233,168,75,0.22)", background:"rgba(233,168,75,0.07)", fontSize:10, fontWeight:500, color:G, letterSpacing:".05em" }}>
        ...
        <p style={{ fontSize:14, color:"#8C8BAA", maxWidth:380, margin:"0 auto 36px", lineHeight:1.7 }}>
        ...
        <div style={{ display:"flex", gap:10, justifyContent:"center", marginBottom:52 }}>
          <Link href="/preischeck" style={{ padding:"12px 26px",borderRadius:10,fontSize:13,fontWeight:600,background:G,color:"#09070E",textDecoration:"none",boxShadow:"0 0 0 1px rgba(233,168,75,0.35),0 4px 24px rgba(233,168,75,0.2)" }}>Preis checken</Link>
        ...
        <div style={{ display:"inline-grid", gridTemplateColumns:"repeat(4,1fr)", background:S.bg, border:`1px solid ${S.br}`, borderRadius:14, overflow:"hidden" }}>
          {[{n:stats.cards.toLocaleString("de-DE"),l:"Karten"},{n:"200",l:"Sets"},{n:stats.users.toLocaleString("de-DE"),l:"Nutzer"},{n:stats.forum.toLocaleString("de-DE"),l:"Forum-Posts"}].map((s,i)=>(
      ...
      {cards.length > 0 && (
        <section style={{ padding:"36px 24px" }}>
          <div style={{ display:"flex",alignItems:"baseline",justifyContent:"space-between",marginBottom:20 }}>
            <h2 style={{ fontSize:15,fontWeight:500,letterSpacing:"-.02em",color:"#EDEAF6",margin:0 }}>Meistgesucht</h2>
            ...
            {(cards as any[]).map(card => {
              ...
              return (
                <Link key={card.id} href={`/preischeck?q=${encodeURIComponent(card.name)}`} style={{ textDecoration:"none" }}>
                  <div style={{ background:S.bg,border:`1px solid rgba(255,255,255,0.055)`,borderRadius:13,overflow:"hidden",transition:"transform .2s,border-color .2s,box-shadow .2s" }}>
                    ...
                    <div style={{ padding:"9px 11px 12px" }}>
                      <div style={{ display:"flex",gap:2,marginBottom:5 }}>
                        {[...Array(dots==="✦✦✦"?6:dots==="✦"?4:dots==="●●"?3:2)].map((_,i)=>(
                          <div key={i} style={{ width:3.5,height:3.5,borderRadius:"50%",background:tc,opacity:i>=(dots==="✦✦✦"?4:dots==="✦"?3:2)?1:0.4,boxShadow:i>=(dots==="✦✦✦"?4:3)?`0 0 4px ${tc}80`:undefined }}/>
                        ))}
                      </div>
                      ...
                      <div style={{ fontSize:9,color:"#474664",marginBottom:6 }}>{String(card.set_id).toUpperCase()} · #{card.number}</div>
                      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                        <span style={{ fontSize:14,fontWeight:600,fontFamily:"monospace",color:G }}>{price}</span>
                        {pct!==null&&<span style={{ fontSize:9,fontWeight:600,color:pct>=0?"#4DBF85":"#E8495A" }}>{pct>=0?"▲":"▼"}{Math.abs(pct).toFixed(1)}%</span>}
                      </div>
                    </div>
                  </div>
                </Link>
      ...
      {/* ══ SCANNER + PORTFOLIO (2-Spalten) ═════════════════════ */}
      <section style={{ padding:"36px 24px" }}>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>

          {/* Scanner links */}
          <div style={{ background:S.bg,border:`1px solid ${S.br}`,borderRadius:20,padding:"24px 24px 24px",position:"relative",overflow:"hidden",display:"grid",gridTemplateColumns:"1fr auto",gap:20,alignItems:"center" }}>
            ...
            <div>
              <div style={{ fontSize:9,fontWeight:600,letterSpacing:".12em",textTransform:"uppercase",color:"#474664",marginBottom:8 }}>KI-Scanner · Gemini Flash</div>
              <div style={{ fontSize:20,fontWeight:400,letterSpacing:"-.025em",lineHeight:1.25,marginBottom:8,color:"#EDEAF6" }}>Foto machen.<br/>Preis wissen.</div>
              <div style={{ fontSize:12,color:"#8C8BAA",lineHeight:1.6,marginBottom:14 }}>KI erkennt deine Karte in Sekunden und zeigt den aktuellen Cardmarket-Wert.</div>
              <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                <span style={{ padding:"3px 9px",borderRadius:5,fontSize:10,fontWeight:500,background:"rgba(233,168,75,0.06)",color:G,border:"1px solid rgba(233,168,75,0.18)" }}>5/Tag Free</span>
                <span style={{ fontSize:10,color:"#474664" }}>Unlimitiert mit Premium ✦</span>
              </div>
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:8,flexShrink:0 }}>
              <div style={{ width:110,aspectRatio:"1",borderRadius:14,background:"rgba(255,255,255,0.02)",border:"1px dashed rgba(233,168,75,0.2)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8 }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ opacity:.22 }}>
                  <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/>
                </svg>
                <span style={{ fontSize:9,color:"#474664",textAlign:"center",lineHeight:1.4 }}>Foto ablegen<br/>oder klicken</span>
              </div>
              <Link href="/scanner" style={{ display:"block",textAlign:"center",padding:"9px",borderRadius:9,background:G,color:"#09070E",fontSize:12,fontWeight:600,textDecoration:"none",letterSpacing:"-.01em",boxShadow:"0 2px 12px rgba(233,168,75,0.2)" }}>Jetzt scannen</Link>
            </div>
          </div>

          {/* Portfolio / Sparkline rechts */}
          <div style={{ background:S.bg,border:`1px solid ${S.br}`,borderRadius:20,padding:22,position:"relative",overflow:"hidden" }}>
            <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,rgba(233,168,75,0.35),transparent)" }}/>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4 }}>
              <div>
                <div style={{ fontSize:10,color:"#474664",marginBottom:4 }}>Mein Portfolio</div>
                <div style={{ fontSize:36,fontWeight:550,letterSpacing:"-.045em",color:"#EDEAF6",lineHeight:1 }}>2.847€</div>
                <div style={{ display:"inline-flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:5,marginTop:6,fontSize:11,fontWeight:500,color:"#4DBF85",background:"rgba(77,191,133,0.08)",border:"1px solid rgba(77,191,133,0.16)" }}>▲ +18,4% · 30 Tage</div>
              </div>
              <div style={{ display:"flex",gap:2 }}>
                {["7T","30T","90T"].map((t,i)=>(
                  <div key={t} style={{ padding:"3px 8px",borderRadius:5,fontSize:10,fontWeight:500,color:i===1?"#EDEAF6":"#474664",background:i===1?"rgba(255,255,255,0.08)":"transparent",cursor:"pointer" }}>{t}</div>
                ))}
              </div>
            </div>
            {/* Sparkline SVG */}
            <svg width="100%" height="56" viewBox="0 0 600 56" preserveAspectRatio="none" style={{ margin:"14px 0",display:"block" }}>
              <defs>
                <linearGradient id="sg" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#E9A84B" stopOpacity=".2"/>
                  <stop offset="100%" stopColor="#E9A84B" stopOpacity="0"/>
                </linearGradient>
              </defs>
              <path d="M0 42 C50 40,80 38,120 33 S175 25,215 20 S270 15,310 13 S365 9,405 7 S460 4,500 3 S560 2,600 1 L600 56 L0 56Z" fill="url(#sg)"/>
              <path d="M0 42 C50 40,80 38,120 33 S175 25,215 20 S270 15,310 13 S365 9,405 7 S460 4,500 3 S560 2,600 1" fill="none" stroke="#E9A84B" strokeWidth="1.5" opacity=".75"/>
            </svg>
            <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7 }}>
              {[{l:"Karten",v:"47"},{l:"Bester Gewinn",v:"+340€",c:"#4DBF85"},{l:"Wunschliste",v:"12"}].map(s=>(
                <div key={s.l} style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.055)",borderRadius:9,padding:"10px 12px" }}>
                  <div style={{ fontSize:9.5,color:"#474664",marginBottom:3 }}>{s.l}</div>
                  <div style={{ fontSize:15,fontWeight:550,color:s.c??"#EDEAF6" }}>{s.v}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ padding:"36px 24px" }}>
        <div style={{ display:"flex",alignItems:"baseline",justifyContent:"space-between",marginBottom:16 }}>
          <h2 style={{ fontSize:15,fontWeight:500,letterSpacing:"-.02em",color:"#EDEAF6",margin:0 }}>Forum</h2>
        ...
        <div style={{ background:S.bg,border:`1px solid ${S.br}`,borderRadius:20,overflow:"hidden" }}>
              ...
              <Link key={post.id} href={`/forum/post/${post.id}`} style={{ textDecoration:"none" }}>
                <div style={{ display:"flex",alignItems:"flex-start",gap:13,padding:"16px 22px",borderBottom:i<posts.length-1?"1px solid rgba(255,255,255,0.04)":"none" }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    ...
                    <div style={{ fontSize:13,fontWeight:500,color:"#EDEAF6",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",letterSpacing:"-.01em",marginBottom:4 }}>{post.title}</div>
...
'@
[System.IO.File]::WriteAllText("$root\src\app\page.tsx", $page_tsx, $enc)
...
'@
[System.IO.File]::WriteAllText("$root\src\components\ui\HeroClient.tsx", $heroclient, $enc)
Write-Host "  OK HeroClient.tsx" -ForegroundColor Green
...
Write-Host ""
Write-Host "Komponenten:" -ForegroundColor Yellow
...
'@
[System.IO.File]::WriteAllText("$root\src\components\ui\CardDetail.tsx", $cardDetail, $enc)
Write-Host "  OK CardDetail.tsx" -ForegroundColor Green
...
Write-Host "  OK PremiumSection.tsx" -ForegroundColor Green
...
'@
[System.IO.File]::WriteAllText("$root\src\app\top-movers\page.tsx", $topMovers, $enc)
Write-Host "  OK page.tsx" -ForegroundColor Green
...
'@
[System.IO.File]::WriteAllText("$root\src\components\ui\PriceTicker.tsx", $priceTicker, $enc)
...
'@
[System.IO.File]::WriteAllText("$root\src\components\ui\PokeballLoader.tsx", $pokeLoader, $enc)
Write-Host "  OK PokeballLoader.tsx" -ForegroundColor Green
...
'@
[System.IO.File]::WriteAllText("$root\src\app\forum\page.tsx", $forumList, $enc)
Write-Host "  OK page.tsx" -ForegroundColor Green
...
'@
$profPath = $root + "\src\app\profil\[username]\page.tsx"
[System.IO.File]::WriteAllText($profPath, $profilePage, $enc)
...
Write-Host "  OK pokedax-logo.png" -ForegroundColor Green

Write-Host ""
Write-Host "=======================================" -ForegroundColor Cyan
Write-Host "v44 fertig!" -ForegroundColor Cyan
Write-Host "=======================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Startseite jetzt exakt nach Vorlage:" -ForegroundColor Green
Write-Host "  1. Hero + Stats-Strip" -ForegroundColor White
Write-Host "  -- duenne Trennlinie --" -ForegroundColor DarkGray
Write-Host "  2. Trending Cards + Preis-Schilder + Rarity-Dots" -ForegroundColor White
Write-Host "  -- duenne Trennlinie --" -ForegroundColor DarkGray
Write-Host "  3. Scanner (links) + Portfolio-Chart (rechts)" -ForegroundColor White
Write-Host "  -- duenne Trennlinie --" -ForegroundColor DarkGray
Write-Host "  4. Forum mit echten Posts aus DB" -ForegroundColor White
Write-Host "  -- duenne Trennlinie --" -ForegroundColor DarkGray
Write-Host "  5. Pricing: Free / Premium (Gold) / Haendler" -ForegroundColor White
Write-Host ""
Write-Host "GitHub Desktop -> Commit 'v44: complete homepage'" -ForegroundColor Yellow
Write-Host "-> Push -> Vercel (~2 Min)" -ForegroundColor White
Write-Host ""
