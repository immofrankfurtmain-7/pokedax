...
const FONT_URL = "https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;700&family=Instrument+Sans:wght@400;500;600&display=swap";
...
export default function CardDetailPage() {
    ...
    async function load() {
      const [{ data: c }, { data: l }] = await Promise.all([
        ...
        SB.from("marketplace_listings")
          ...
          .eq("card_id", id).eq("is_active", true).order("price", { ascending: true }).limit(8),
      ]);
      setCard(c);
      ...
      const { data: hist } = await SB.from("price_history").select("date,price").eq("card_id", id).order("date",{ascending:true}).limit(90);
      if (hist?.length) setHistory(hist);
      const { data: salesData } = await SB.from("escrow_transactions").select("amount,created_at,condition,profiles!escrow_transactions_buyer_id_fkey(username)").eq("card_id", id).eq("status","released").order("created_at",{ascending:false}).limit(10);
  ...
  if (loading) return (
    <div style={{ background: BG, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: GD2 }}>Lade…</div>
  );
  if (!card) return (
    <div style={{ background: BG, minHeight: "100vh", display: "flex", flexDirection:"column", alignItems: "center", justifyContent: "center", gap:16 }}>
      <div style={{color:TX2}}>Karte nicht gefunden</div>
      <Link href="/preischeck" style={{color:GOLD}}>← Zurück</Link>
    </div>
  );
    ...
    <div style={{ background: BG, minHeight: "100vh", color: TX }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "clamp(60px,8vw,100px) clamp(20px,4vw,48px)" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"380px 1fr", gap:64, alignItems:"start" }}>
          ...
          <div>
            <div style={{ background:BG2, borderRadius:24, overflow:"hidden", border:"1px solid rgba(201,166,107,0.15)", boxShadow:"0 40px 80px rgba(0,0,0,0.6)", marginBottom:24, position:"relative" }}>
...
function PriceChart({ data }: { data: { date: string; price: number }[] }) {
  ...
  const pts = data.map((d, i) => {
    const x = pl + (n > 1 ? i / (n - 1) : 0) * iw;
    const y = pt + (1 - (d.price - minP) / range) * ih;
  ...
  const lx     = last[0];
  const ly     = last[1];
  const areaD  = "M " + pts[0] + " L " + ptsStr + " L " + (pl + iw) + "," + (pt + ih) + " L " + pl + "," + (pt + ih) + " Z";
  const first  = data[0];
  const lastD  = data[data.length - 1];
  const up     = lastD.price >= first.price;
  const clr    = up ? "#3db87a" : "#dc4a5a";
  const vb     = "0 0 " + w + " " + h;
  const diff   = ((lastD.price - first.price) / first.price * 100).toFixed(1);
  ...
  return (
    <div>
      <svg viewBox={vb} style={{ width:"100%", height:"auto" }}>
        ...
        <path d={pathD} fill="none" stroke={clr} strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round"/>
        <circle cx={lx} cy={ly} r={3} fill={clr}/>
      </svg>
      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
        <div style={{ fontSize:11, color:"rgba(237,233,224,0.4)" }}>{minP.toFixed(2)} €</div>
        <div style={{ fontSize:12, fontWeight:600, color:clr }}>{up?"▲":"▼"} {Math.abs(lastD.price-first.price).toFixed(2)} € ({diff}%)</div>
