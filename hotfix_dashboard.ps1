# Hotfix: Dashboard Navbar-Link + Auth Fix
...
Write-Host "  OK  Navbar.tsx (Dashboard-Link hinzugefuegt)" -ForegroundColor Green
...
export default function DashboardPage() {
  ...
  useEffect(()=>{
    const sb = createClient();
    // Use getSession first (reads local storage, no network call)
    sb.auth.getSession().then(async({data:{session}})=>{
      if (!session?.user) {
        // Try getUser as fallback (network call)
        const { data: { user } } = await sb.auth.getUser();
        if (!user) { window.location.href="/auth/login"; return; }
        setUser(user);
        await Promise.all([
          loadProfile(sb, user.id),
          loadPortfolio(sb, user.id),
          loadMatches(sb, user.id),
          loadListings(sb, user.id),
          loadScans(sb, user.id),
          loadFantasy(sb, user.id),
        ]);
        setLoading(false);
        return;
      }
...
Write-Host "  OK  dashboard/page.tsx (Auth-Fix)" -ForegroundColor Green

Write-Host ""
Write-Host "GitHub Desktop -> Commit 'fix: dashboard nav + auth' -> Push" -ForegroundColor Yellow
