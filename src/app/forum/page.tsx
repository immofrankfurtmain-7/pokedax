  ...
  return (
    <div style={{ background: BG, minHeight: "100vh", color: TX }}>
      <style dangerouslySetInnerHTML={{ __html: `
        .ph{font-family:'Playfair Display',serif;letter-spacing:-.05em}
        .post-row{background:#111;border:1px solid rgba(255,255,255,.06);border-radius:16px;overflow:hidden;transition:border-color .2s,transform .2s;cursor:pointer;display:flex}
        .post-row:hover{border-color:rgba(201,166,107,.25);transform:translateY(-1px)}
        .cat-btn{padding:9px 18px;border-radius:100px;border:1px solid transparent;font-size:13px;cursor:pointer;transition:all .2s;background:transparent;width:100%;text-align:left}
        .cat-btn.active{background:rgba(201,166,107,.1);border-color:rgba(201,166,107,.3);color:#C9A66B;font-weight:600}
        .cat-btn:not(.active){color:rgba(237,233,224,.5)}
        .cat-btn:not(.active):hover{color:#EDE9E0;background:rgba(255,255,255,.04)}
        .btn-gold{display:inline-flex;align-items:center;gap:8px;padding:12px 24px;background:#C9A66B;color:#0A0A0A;border-radius:100px;border:none;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;transition:transform .2s}
        .btn-gold:hover{transform:scale(1.03)}
        .vote-btn{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;padding:0 20px;background:transparent;border:none;border-right:1px solid rgba(255,255,255,.06);cursor:pointer;min-width:60px;flex-shrink:0;transition:background .15s}
        .vote-btn:hover{background:rgba(201,166,107,.06)}
        .live-dot{width:7px;height:7px;border-radius:50%;background:#3db87a;animation:lp 2s ease-in-out infinite}
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "clamp(60px,8vw,100px) clamp(20px,4vw,48px)" }}>
        <div style={{ marginBottom: 56, display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 20 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: GD2, marginBottom: 16 }}>Community</div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button onClick={() => { if (!user) { window.location.href = "/auth/login"; return; } setShowNew(true); }} className="btn-gold">
              + Beitrag erstellen
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 48, alignItems: "start" }}>
          <div style={{ position: "sticky", top: 100 }}>
            ...
            <div style={{ marginTop: 28, padding: "20px", background: BG2, borderRadius: 16, border: "1px solid rgba(201,166,107,0.1)" }}>
              <div style={{ fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: GD2, marginBottom: 8 }}>Community</div>
              <div className="ph" style={{ fontSize: 28, fontWeight: 500, color: GOLD }}>{posts.length}</div>
            ...
            {loading ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {Array.from({length:5}).map((_,i) => (
                  <div key={i} style={{ height: 90, background: BG2, borderRadius: 16, opacity: 0.4 }}/>
                ))}
              </div>
            ) : posts.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {posts.map((post: any, i: number) => {
                  ...
                  return (
                    <div key={post.id} className={"post-row fade-up"} style={{ animationDelay: i*30 + "ms" }}>
                      ...
                      <Link href={"/forum/post/" + post.id} style={{ textDecoration: "none", flex: 1, padding: "16px 20px", display: "flex", gap: 14 }}>
                        {imgSrc && (
                          <div style={{ width: 40, height: 56, borderRadius: 6, overflow: "hidden", background: "#1A1A1A", flexShrink: 0 }}>
...
function NewPostModal({ cats, onClose, onCreated }: { cats: any[]; onClose: ()=>void; onCreated: ()=>void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }}
      ...
      <div style={{ background: "#0F0F0F", borderRadius: 24, width: "100%", maxWidth: 520, padding: 36, position: "relative", border: "1px solid rgba(201,166,107,0.2)" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input value={title} onChange={e => setTitle(e.target.value)} maxLength={120} placeholder="Worum geht es?"
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <textarea value={body} onChange={e => setBody(e.target.value)} rows={4} placeholder="Schreib mehr dazu…"
            style={{ width: "100%", padding: "14px 18px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: TX, fontSize: 15, outline: "none", resize: "vertical", fontFamily: "inherit" }}/>
        </div>

        {error && <div style={{ marginBottom: 16, fontSize: 13, color: "#dc4a5a" }}>{error}</div>}

        <button onClick={submit} disabled={loading || !title.trim()} className="btn-gold"
          style={{ width: "100%", justifyContent: "center", padding: "16px", fontSize: 15, opacity: loading || !title.trim() ? 0.5 : 1, borderRadius: 100 }}>
