...
export default function CardDetailPage() {
  ...
  const [history,  setHistory]  = useState<{recorded_at?:string;date?:string;price:number}[]>([]);
          ...
          <div style={{ marginTop: 48 }}>
            ...
            <div style={{ background: BG2, borderRadius: 20, padding: "24px", border: "1px solid rgba(201,166,107,0.1)" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, fontSize: 11, color: "rgba(237,233,224,0.35)" }}>
                <span>{history[0]?.recorded_at || history[0]?.date}</span>
                <span>{history[history.length-1]?.recorded_at || history[history.length-1]?.date}</span>
              </div>
            </div>
          </div>
        )}
...
function PriceChart({ data }: { data: { recorded_at?: string; date?: string; price: number }[] }) {
