...
const FALLBACK: TickerCard[] = [
  ...
  { name: "Pikachu VMAX", price: 142.80, change: 12.7, set: "SV9" },
...
export default function PriceTicker() {
  ...
  useEffect(() => {
    // Hier kannst du später echte API-Daten laden
  }, []);
  ...
  return (
    <div className="bg-[var(--bg-1)] border-b border-[var(--br-1)] py-3 overflow-hidden">
      <div className="max-w-screen-2xl mx-auto px-10 flex items-center gap-8 text-xs text-[var(--g)] whitespace-nowrap overflow-hidden">
        ...
        >
          {items.map((card, i) => (
            <span key={`${card.name}-${i}`} className="flex items-center gap-6 flex-shrink-0">
              ...
              <span className="text-[var(--tx-3)]">{card.set}</span>
              <span className="price">{card.price.toFixed(2)} €</span>
              <span className={`flex items-center gap-1 ${card.change >= 0 ? "text-emerald-400" : "text-red-400"}`}>
