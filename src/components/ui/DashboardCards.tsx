...
// ── POKEMON SPARKLINE ─────────────────────────────────────────────────────
function PokemonSparkline({ card }: { card: CollectionCard }) {
  const p30 = card.price_avg30 ?? card.price_market ?? 0
  const p7  = card.price_avg7  ?? card.price_market ?? 0
  const p1  = card.price_avg1  ?? card.price_market ?? 0
  const cur = card.price_market ?? 0

  const points = [p30, p7, p1, cur]
  ...
  const minP = Math.min(...points.filter(p => p > 0)) * 0.95
  const maxP = Math.max(...points) * 1.05
  const range = maxP - minP || 1

  const W = 72, H = 28
  const xs = [4, 26, 50, 68]
  const ys = points.map(p => H - ((p - minP) / range) * (H - 4) - 2)

  const pathD = `M${xs[0]} ${ys[0]} C${xs[0]+10} ${ys[0]},${xs[1]-6} ${ys[1]},${xs[1]} ${ys[1]} C${xs[1]+8} ${ys[1]},${xs[2]-8} ${ys[2]},${xs[2]} ${ys[2]} C${xs[2]+6} ${ys[2]},${xs[3]-8} ${ys[3]},${xs[3]} ${ys[3]}`
  const areaD = `${pathD} L${xs[3]} ${H} L${xs[0]} ${H} Z`

  const gradId = `g-${card.id.replace(/[^a-z0-9]/gi, '')}`

  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3, flexShrink: 0 }}>
      <div style={{ fontSize: 15, fontWeight: 900, color: '#fff', letterSpacing: '-.02em' }}>
        €{cur.toFixed(2)}
      </div>

      {hasData && (
        <div style={{ borderRadius: 10, padding: '4px 6px 2px', background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.06)' }}>
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
            ...
            <circle cx={xs[0]} cy={ys[0]} r="2"   fill={color} opacity=".35"/>
            <circle cx={xs[1]} cy={ys[1]} r="2"   fill={color} opacity=".55"/>
            <circle cx={xs[2]} cy={ys[2]} r="2"   fill={color} opacity=".75"/>
            <circle cx={xs[3]} cy={ys[3]} r="3.5" fill={color}/>
            {isUp   && <polygon points={`${xs[3]},-3 ${xs[3]+3},2 ${xs[3]-3},2`}   fill={color} opacity=".9"/>}
            {isDown && <polygon points={`${xs[3]},${H+5} ${xs[3]+3},${H} ${xs[3]-3},${H}`} fill={color} opacity=".9"/>}
            {!isUp && !isDown && <line x1={xs[3]-4} y1={ys[3]-4} x2={xs[3]+4} y2={ys[3]-4} stroke={color} strokeWidth="1.5" strokeLinecap="round" opacity=".8"/>}
          </svg>
        </div>
      )}
      ...
      <div style={{
        ...
        {isUp   && <svg width="6" height="6" viewBox="0 0 6 6"><polygon points="3,0 6,6 0,6" fill={color}/></svg>}
        {isDown && <svg width="6" height="6" viewBox="0 0 6 6"><polygon points="3,6 6,0 0,0" fill={color}/></svg>}
        {!isUp && !isDown && <svg width="7" height="3" viewBox="0 0 7 3"><rect width="7" height="2" rx="1" y=".5" fill={color}/></svg>}
        {pctChange > 0 ? '+' : ''}{pctChange.toFixed(1)}% · 30T
      </div>
    </div>
  )
}
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ width: '100%', maxWidth: 600, maxHeight: '88vh', borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column', background: 'rgba(7,2,26,0.99)', border: '1.5px solid rgba(167,139,250,.3)', boxShadow: '0 25px 60px rgba(0,0,0,.7)' }}
      >
        {/* Header */}
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {sorted.length === 0 ? (
            ...
            const accent = typeColor(card.rarity)
            return (
              <motion.div
                key={card.id}
              ...
              >
                ...
                {/* Pokéball Hintergrund */}
                <svg style={{ position: 'absolute', right: -10, top: '50%', transform: 'translateY(-50%)', width: 55, height: 55, opacity: .04 }} viewBox="0 0 60 60">
                ...
                {/* Rang */}
                <span style={{ color: 'rgba(255,255,255,.15)', fontSize: 11, fontFamily: 'monospace', width: 18, textAlign: 'right', flexShrink: 0 }}>{i + 1}</span>

                {/* Bild */}
                {card.image_url ? (
                  <img src={card.image_url} alt={card.name} style={{ width: 36, height: 50, objectFit: 'cover', borderRadius: 7, flexShrink: 0, border: `1px solid ${accent}44` }}/>
                ) : (
                  <div style={{ width: 36, height: 50, borderRadius: 7, background: 'rgba(255,255,255,.05)', flexShrink: 0, border: `1px solid ${accent}33` }}/>
                )}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 5, alignItems: 'center', marginTop: 3 }}>
                    <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 8, background: `${accent}18`, color: accent }}>#{card.number}</span>
                    {card.set_id && <span style={{ color: 'rgba(255,255,255,.2)', fontSize: 10 }}>{card.set_id}</span>}
                  </div>
                  {card.rarity && <div style={{ color: 'rgba(255,255,255,.2)', fontSize: 10, marginTop: 2 }}>{card.rarity}</div>}
                </div>

                {/* Sparkline */}
                <PokemonSparkline card={card}/>
              </motion.div>
            )
          })}
        </div>
