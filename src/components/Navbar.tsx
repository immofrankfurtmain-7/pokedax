          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }} className="desktop-auth">
            {!user ? (
              <>
                ...
                onMouseEnter={e => { (e.currentTarget as any).style.transform = "scale(1.04)"; (e.currentTarget as any).style.boxShadow = "0 8px 32px rgba(201,166,107,0.4)"; }}
                onMouseLeave={e => { (e.currentTarget as any).style.transform = "none"; (e.currentTarget as any).style.boxShadow = "0 4px 20px rgba(201,166,107,0.3)"; }}>
                  Kostenlos starten
                </Link>
              </>
            ) : (
                ...
                {dropOpen && (
                  <div style={{
                    ...
                    <div style={{ padding: "14px 16px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      ...
                      <div style={{ fontSize: 11, color: "rgba(237,233,224,0.4)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.email}</div>
                    </div>
                    <div style={{ padding: "6px 0" }}>
                      {DROPDOWN_LINKS.map(({ href, label, icon }) => (
                        <Link key={href} href={href} onClick={() => setDropOpen(false)} style={{
                          display: "flex", alignItems: "center", gap: 10,
                          padding: "9px 16px", fontSize: 13, color: TX2,
                          textDecoration: "none", transition: "background 0.1s, color 0.1s",
                        }}
                        onMouseEnter={e => { (e.currentTarget as any).style.background = "rgba(201,166,107,0.06)"; (e.currentTarget as any).style.color = TX; }}
                        onMouseLeave={e => { (e.currentTarget as any).style.background = "transparent"; (e.currentTarget as any).style.color = TX2; }}>
                          <span style={{ fontSize: 11, color: GOLD, width: 14, textAlign: "center" }}>{icon}</span>
                          {label}
                        </Link>
                      ))}
                    </div>
                    <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "6px 0" }}>
                      ...
                      onMouseLeave={e => (e.currentTarget as any).style.background = "transparent"}>
                        <span style={{ fontSize: 11, color: "rgba(237,233,224,0.3)", width: 14, textAlign: "center" }}>×</span>
                        Abmelden
                      </button>
                    </div>
                  </div>
          ...
          <div style={{
            ...
            {NAV_LINKS.map(({ href, label }) => (
              <Link key={href} href={href} style={{
                ...
                color: TX, textDecoration: "none",
                borderBottom: "1px solid rgba(201,166,107,0.08)",
              }}>{label}</Link>
            ))}
            <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
              {user ? (
                <Link href="/dashboard" style={{ padding: "12px", borderRadius: 100, background: GOLD, color: BG, textDecoration: "none", textAlign: "center", fontWeight: 600 }}>
                  Dashboard
                </Link>
              ) : (
                <>
                  <Link href="/auth/login" style={{ padding: "12px", borderRadius: 100, border: `1px solid ${GOLD}`, color: GOLD, textDecoration: "none", textAlign: "center" }}>
                    Anmelden
                  </Link>
                  <Link href="/auth/register" style={{ padding: "12px", borderRadius: 100, background: GOLD, color: BG, textDecoration: "none", textAlign: "center", fontWeight: 600 }}>
                    Kostenlos starten
                  </Link>
                </>
              )}
