# ═══════════════════════════════════════════════════════════
# PokéDax v5 — Weltklasse Redesign
# Satoshi · Inter · DM Mono · Gold #E9A84B · Canvas #0a0a0a
# Ziel: 98+/100
# ═══════════════════════════════════════════════════════════
...
Write-Host ""
Write-Host "pokEdax v5 — Weltklasse Redesign" -ForegroundColor Cyan
...
$dirs = @(
  ...
  "$root\src\app\auth\login","$root\src\app\auth\register",
  ...
  "$root\src\app\impressum","$root\src\app\datenschutz","$root\src\app\agb"
)
foreach ($d in $dirs) {
...
Write-Host "Core (Satoshi + Gold-System):" -ForegroundColor Yellow
...
:root {
  ...
  --gold-12: rgba(233,168,75,0.12);
  --gold-08: rgba(233,168,75,0.08);
  --gold-05: rgba(233,168,75,0.05);
...
::-webkit-scrollbar-thumb { background: var(--br-2); border-radius: 2px; }
...
@keyframes price-flash   { 0%,100%{background:transparent} 50%{background:rgba(233,168,75,0.1)} }
@keyframes card-shimmer  {
  0%   { transform: translateX(-100%) rotate(35deg); }
  100% { transform: translateX(200%)  rotate(35deg); }
}
@keyframes pulse-gold {
  0%,100% { box-shadow: 0 0 0 0 rgba(233,168,75,0); }
  50%     { box-shadow: 0 0 20px 4px rgba(233,168,75,0.15); }
}
...
.card-hover::before {
  ...
  background: radial-gradient(circle at 50% 0%, rgba(233,168,75,0.07), transparent 65%);
  opacity: 0;
...
.card-hover:hover {
  ...
  border-color: rgba(233,168,75,0.18) !important;
  box-shadow:
    0 18px 48px rgba(0,0,0,0.65),
    0 0 0 1px rgba(233,168,75,0.06),
    0 0 32px rgba(233,168,75,0.05);
}
...
.gold-glow::before {
  ...
  inset: -1px;
  border-radius: inherit;
  background: radial-gradient(ellipse at 50% 0%, rgba(233,168,75,0.3), transparent 60%);
  opacity: 0;
  transition: opacity .2s var(--ease);
  pointer-events: none;
  z-index: -1;
}
.gold-glow:hover { transform: translateY(-1px); }
.gold-glow:hover::before { opacity: 1; }

/* ── Section divider ── */
.section-divider { height: 1px; background: var(--br-1); margin: 0 24px; }
...
Write-Host ""
Write-Host "API + Components (unveraendert):" -ForegroundColor Yellow
...
Write-Host "  OK  impressum/page.tsx" -ForegroundColor Green

$datenschutz = @'
import Link from "next/link";
export const metadata = { title:"Datenschutz" };
const TX1="#f0f0f5",TX2="#a8a8b8",TX3="#6b6b7a";
const BG1="#111113",BR2="rgba(255,255,255,0.085)",BR1="rgba(255,255,255,0.050)";
export default function DatenschutzPage() {
  const sections=[
    ...
    {h:"Zweck der Verarbeitung",t:"Die Daten dienen ausschließlich der Bereitstellung des PokéDax-Services."},
    {h:"Speicherung",t:"Daten werden auf EU-Servern (Supabase) gespeichert. Passwörter als bcrypt-Hash."},
    {h:"Weitergabe",t:"Keine Weitergabe personenbezogener Daten an Dritte."},
    {h:"Ihre Rechte",t:"Auskunft, Berichtigung, Löschung (Art. 15–18 DSGVO). Anfragen an: hello@pokedax.de"},
...
Write-Host "  OK  datenschutz/page.tsx" -ForegroundColor Green

Write-Host ""
Write-Host "==================================" -ForegroundColor Cyan
Write-Host "v5 fertig!" -ForegroundColor Cyan
Write-Host "==================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Design-System v5 (98+/100):" -ForegroundColor Yellow
Write-Host "  Canvas: #0a0a0a (tiefstes Schwarz)" -ForegroundColor White
Write-Host "  Satoshi 300 fuer Headlines + Logo" -ForegroundColor White
Write-Host "  DM Mono fuer alle Preise und Zahlen" -ForegroundColor White
Write-Host "  Gold #E9A84B als einziger Akzent" -ForegroundColor White
Write-Host "  Ticker: komplett gold, kein Cyan/Gruen" -ForegroundColor White
Write-Host "  Karten: card-hover mit radial glow" -ForegroundColor White
Write-Host "  18 Seiten konsistent" -ForegroundColor White
Write-Host ""
Write-Host "Seiten:" -ForegroundColor Yellow
Write-Host "  / preischeck scanner portfolio fantasy" -ForegroundColor White
Write-Host "  forum leaderboard settings" -ForegroundColor White
Write-Host "  dashboard/premium (Conversion)" -ForegroundColor White
Write-Host "  auth/login auth/register" -ForegroundColor White
Write-Host "  impressum datenschutz 404" -ForegroundColor White
Write-Host ""
Write-Host "Supabase/Stripe/APIs unveraendert" -ForegroundColor DarkGray
Write-Host ""
Write-Host "GitHub Desktop -> Commit 'v5: worldclass redesign'" -ForegroundColor Yellow
Write-Host "-> Push -> Vercel (~3 Min)" -ForegroundColor White
Write-Host ""
