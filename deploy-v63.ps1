# PokéDax v6.3 — Auth Fix · Forum Fix · Mobile · Impressum
$root = "C:\Users\lenovo\pokedax\pokedax\pokedax"
$enc  = New-Object System.Text.UTF8Encoding $true
Write-Host ""
Write-Host "pokEdax v6.3 — Logik + Mobile Fixes" -ForegroundColor Yellow
...
$dirs = @(
  ...
  "$root\src\app\auth\login","$root\src\app\auth\register","$root\src\app\auth\callback",
...
body {
  ...
  background-image:
    radial-gradient(circle at 20% 25%, rgba(233,168,75,0.13) 0%, transparent 62%),
    radial-gradient(circle at 80% 80%, rgba(233,168,75,0.10) 0%, transparent 68%),
    radial-gradient(circle at 45% 95%, rgba(233,168,75,0.07) 0%, transparent 55%);
...
::-webkit-scrollbar-thumb { background: rgba(233,168,75,0.2); border-radius: 2px; }
...
/* ═══════════════════════════════════════════════════
   GOLD GLOW — signature effect, exact reference
   ═══════════════════════════════════════════════════ */
.gold-glow {
...
.gold-glow::before {
  ...
  position: absolute;
  inset: -130%;
  background: radial-gradient(circle at center, rgba(233,168,75,0.52) 0%, transparent 85%);
  opacity: 0;
  transition: all 0.8s cubic-bezier(0.23, 1, 0.32, 1);
  z-index: 0;
...
.gold-glow:hover::before { opacity: 1; transform: scale(1.6); }
.gold-glow:hover {
  transform: translateY(-10px);
  box-shadow: 0 0 0 1px #E9A84B, 0 65px 140px -45px rgba(233,168,75,0.75);
}
.gold-glow > * { position: relative; z-index: 1; }

/* card-hover same system */
.card-hover {
  transition: all 0.8s cubic-bezier(0.23, 1, 0.32, 1);
  position: relative;
  overflow: hidden;
...
.card-hover:hover::before { opacity: 1; transform: scale(1.6); }
.card-hover:hover {
  transform: translateY(-10px);
  box-shadow: 0 0 0 1px #E9A84B, 0 65px 140px -45px rgba(233,168,75,0.75);
}
...
@media (max-width: 768px) {
  ...
  h1 { letter-spacing: -.06em !important; }
  /* Stack grids on mobile */
  .mobile-stack {
    grid-template-columns: 1fr !important;
  }
  /* Reduce gold-glow on mobile (heavy on performance) */
  .gold-glow:hover {
    transform: translateY(-4px);
    box-shadow: 0 0 0 1px #E9A84B, 0 20px 40px -10px rgba(233,168,75,0.4);
  }
  .card-hover:hover {
    transform: translateY(-4px);
    box-shadow: 0 0 0 1px #E9A84B, 0 20px 40px -10px rgba(233,168,75,0.4);
  ...
  .ticker-text { font-size: 10px !important; letter-spacing: .08em !important; }
}

@media (max-width: 480px) {
  body { font-size: 15px; }
  .gold-glow:hover, .card-hover:hover { transform: none; box-shadow: 0 0 0 1px rgba(233,168,75,0.4); }
}
...
Write-Host "  OK  CardDetail.tsx" -ForegroundColor DarkGray
...
Write-Host "  OK  PokeballLoader.tsx" -ForegroundColor DarkGray
...
Write-Host "  OK  page.tsx" -ForegroundColor DarkGray
...
Write-Host ""
Write-Host "=====================================" -ForegroundColor Yellow
Write-Host "v6.3 fertig!" -ForegroundColor Yellow
Write-Host "=====================================" -ForegroundColor Yellow
Write-Host ""
Write-Host "Fixes in v6.3:" -ForegroundColor Cyan
Write-Host "  AUTH: Login mit echtem Supabase signInWithPassword" -ForegroundColor White
Write-Host "  AUTH: Register mit echtem signUp + E-Mail-Bestätigung" -ForegroundColor White
Write-Host "  AUTH: Google OAuth mit Callback-Route" -ForegroundColor White
Write-Host "  AUTH: Fehlermeldungen auf Deutsch" -ForegroundColor White
Write-Host "  FORUM: Direkte Supabase-Verbindung (kein @/lib Fehler mehr)" -ForegroundColor White
Write-Host "  FORUM: Error-Handling + Kategorie-Filter" -ForegroundColor White
Write-Host "  IMPRESSUM: Damir Babic, Ricarda-Huch-Str. 15, 60431 Frankfurt" -ForegroundColor White
Write-Host "  MOBILE: CSS-Optimierungen fuer 768px und 480px" -ForegroundColor White
Write-Host "  MOBILE: Reduzierte gold-glow Animation auf Touch-Geraten" -ForegroundColor White
Write-Host "  MOBILE: Min-height 44px fuer alle Touch-Targets" -ForegroundColor White
Write-Host "  LIB: src/lib/supabase/client.ts erstellt" -ForegroundColor White
Write-Host ""
Write-Host "GitHub Desktop -> Commit 'v6.3: auth + forum + mobile + impressum'" -ForegroundColor Yellow
